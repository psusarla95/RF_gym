...
class RFBeamEnv2(gym.Env):
    ...
    def step(self, action):
        ...
        SNR = self.mimo.Calc_SNR(self.ptx, self.action_values[action][0], self.action_values[action][1],
                                 self.action_values[action][2], self.action_values[action][3])
        ...
        if done:
            reward = 10
            print(
                "Reached Final state: {0}, Actions Taken: {1}, steps: {2}".format(SNR_state, self.action_values[action],
                                                                                  self.count))
        elif SNR_state <= prev_state:
            reward = -40
        elif SNR_state < self.state_threshold:
